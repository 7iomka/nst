...
function assortimentSliderActions() {
  ...
  function getContentOfSlide($slide, $gallery) {
      ...
      var slideSchemes = $slide.find('.gallery-slide__content-schemes').html();
      ...
      var $contextSchemes = $gallery.find('.gallery-content__schemes');
    ...
    $contextDesc.add($contextSchemes).addClass('short_hidden').afterTransition(function() {
          // change new data
          $contextSchemes.html(slideSchemes);
          var $slideGalleryItems = $contextSchemes.find('[data-for-fancybox]');
          var slideGalleryId = $slideGalleryItems.attr('data-for-fancybox');
          $slideGalleryItems.attr({
            'data-fancybox': slideGalleryId
          }).removeAttr('data-for-fancybox');

          $contextDesc.html(slideDesc);

          // make visible again
          $contextDesc.add($contextSchemes).removeClass('short_hidden');
      });
